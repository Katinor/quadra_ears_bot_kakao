from kakaobot import Kboard, Message
def version(flag):
	temp_board = Kboard()
	if flag == "도움말":
		temp_text = "반가워! 난 사잽이라구 해!\n밑의 리스트들을 누르면 원하는 도움말을 찾을 수 있어!"
		temp_board.add_button("도움말 약관")
		temp_board.add_button("도움말 일상")
		temp_board.add_button("도움말 사용자")
		temp_board.add_button("도움말 게임")
		temp_board.add_button("통상모드로 전환")
	elif "약관" in flag:
		temp_text = "사잽이봇을 이용하기위해 지켜야할 것들이 담겨져있어!"
		temp_board.add_button("이용약관")
		temp_board.add_button("통상모드로 전환")
	else:
		temp_text = "아직은 도움말이 준비되지 않았어!"
	return Message(text = temp_text, keyboard = temp_board)

def tou(flag):
	temp_board = Kboard()
	if flag == "이용약관":
		temp_text = "사잽이봇을 이용하기위해 지켜야할 것들이 담겨져있어!\n꼭 전부 읽어줘~!"
		temp_board.add_button("이용약관 개인정보 관리방침")
		temp_board.add_button("이용약관 저작권")
		temp_board.add_button("이용약관 사용 API")
		temp_board.add_button("이용약관 프로그램 라이센스")
		temp_board.add_button("이용약관 연락처")
		temp_board.add_button("통상모드로 전환")
		...
		temp_text = "https://blog.4ears.net/"
		temp_board.add_button("이용약관")
		temp_board.add_button("통상모드로 전환")
	else:
